...
def _invalid_params():
    ...
    grimelipid_valid_dict = {"A": 10.0, "B": 2.0, "r0": 6.0, "rc": 12.0}
    grimelipid_invalid_dicts = _make_invalid_param_dict(grimelipid_valid_dict)
    invalid_params_list.extend(
        _make_invalid_params(grimelipid_invalid_dicts, md.pair.GrimeLipid, {}))
...
def _valid_params(particle_types=['A', 'B']):
    ...
    grimelipid_valid_param_dicts = _make_valid_param_dicts(grimelipid_arg_dict)
    valid_params_list.append(
        paramtuple(md.pair.GrimeLipid,
                   dict(zip(combos, grimelipid_valid_param_dicts)), {}))
